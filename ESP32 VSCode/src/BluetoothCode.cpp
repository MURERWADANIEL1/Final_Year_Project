...
const float SAMPLE_RATE = 22050.0;     // Hz
...
// Generate a dummy sine wave with a frequency sweep (chirp)
int16_t readMicSample() {
    float t = (float)currentSampleIndex / SAMPLE_RATE;
    float duration = 5.0; // seconds for a full sweep
    float f0 = 200.0;     // Start frequency
    float f1 = 2000.0;    // End frequency

    // Linear frequency sweep (chirp)
    float freq = f0 + (f1 - f0) * fmod(t, duration) / duration;
    float sample = sin(2 * PI * freq * t);

    int16_t value = (int16_t)(sample * 32767);  // 16-bit signed PCM
    currentSampleIndex++;
    return value;
}
